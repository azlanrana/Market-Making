//! S3 backtest for QueueFarmer v3 strategy — NEW ENGINE
//!
//! QueueFarmer v3.0: flow filter removed, 1 bps price-improving spread,
//! book imbalance filter (snapshot-safe). Designed for 0.03 bps real spread
//! → quoting at 1 bps puts us inside the touch, queue_pos = 50% of level.
//!
//! Set S3_BUCKET, S3_PREFIX, AWS_REGION to run. Set TRADING_PAIR for pair config.
//!
//! Run: cargo test -p mm-engine backtest_s3_queue_farmer_v3 --release -- --ignored --nocapture

use mm_engine::{BacktestEngine, SimpleFeeModel};
use queue_farmer_v3::QueueFarmerV3;
...
async fn backtest_s3_queue_farmer_v3() {
    ...
    println!("\n=== QueueFarmer v3 S3 Backtest (NEW ENGINE) ===");
    ...
    // v3: 1 bps spread (live + backtest) — price-improving vs 0.03 bps real spread
    // Book imbalance 0.85 (slightly lenient for more fills at tight spreads)
    let strategy = QueueFarmerV3::new(order_amount)
        .with_backtest_mode()
        .with_book_imbalance(0.85, 3)
    ...
    println!("Strategy: QueueFarmer v3.0 (1 bps price-improving, book imbalance filter, 65/35 inv stop)");
    ...
    match engine.run(loader).await {
        Ok(results) => {
            ...
            println!("=== QueueFarmer v3 S3 Backtest Results (NEW ENGINE) ===");
