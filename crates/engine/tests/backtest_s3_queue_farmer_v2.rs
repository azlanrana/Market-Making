//! S3 backtest for QueueFarmer v2 strategy — NEW ENGINE
//!
//! Uses BacktestEngine (mm-engine). QueueFarmer v2.0: flow imbalance filter,
//! pump/dump defense, simplified (no vol spread, no impulse filter, no daily loss limit).
//!
...
async fn backtest_s3_queue_farmer_v2() {
    ...
    println!("\n=== QueueFarmer v2 S3 Backtest (NEW ENGINE) ===");
    ...
    let strategy = QueueFarmerV2::new(order_amount)
        .with_backtest_mode()
        .with_flow_params(8.0, 0.75, 15.0)
        .with_spread(2.0, 3.5)
    ...
    match engine.run(loader).await {
        Ok(results) => {
            ...
            println!("=== QueueFarmer v2 S3 Backtest Results (NEW ENGINE) ===");
