...
impl SftpLoader {
    ...
    pub fn list_files(&self) -> Result<Vec<String>> {
        ...
        if path_parts.len() >= 6 {
            ...
            if day_idx >= 3 && day_idx < path_parts.len() {
                ...
                if let (Ok(year), Ok(month), Ok(start_day)) = (
                    ...
                    println!("Starting multi-day traversal from {}-{:02}-{:02}", year, month, start_day);
                        ...
                        match sftp.readdir(Path::new(&day_path)) {
                            Ok(files) => {
                                ...
                                for (path, _) in files {
                                    if let Some(name) = path.file_name().and_then(|n| n.to_str()) {
                                        if name.ends_with(".gz") {
                                            ...
                                            let file_path = format!("{:04}/{:02}/{:02}/{}/{}/{}", 
                                                target_year, target_month, target_day, cdc_part, pair_part, name);
                                ...
                                if !day_files.is_empty() {
                                    ...
                                    println!("  {:04}-{:02}-{:02}: No files found", target_year, target_month, target_day);
                                }
                            }
                            Err(e) => {
                                // Day directory doesn't exist or error reading
                                println!("  {:04}-{:02}-{:02}: Directory not found or error: {}", 
                    ...
                    if !all_files.is_empty() {
                        ...
                        all_files.sort_by(|a, b| {
                            let a_ts = a.split('/').last()
                                .and_then(|f| f.strip_suffix(".gz"))
                                .and_then(|f| f.parse::<u64>().ok())
                                .unwrap_or(0);
                            let b_ts = b.split('/').last()
                                .and_then(|f| f.strip_suffix(".gz"))
                                .and_then(|f| f.parse::<u64>().ok())
                                .unwrap_or(0);
                            a_ts.cmp(&b_ts)
        ...
        let files = sftp.readdir(Path::new(&self.remote_path))
    ...
    fn download_and_decompress(&self, remote_filename: &str) -> Result<Vec<u8>> {
        ...
        let data = if remote_filename.ends_with(".gz") {
            let file = std::fs::File::open(&temp_compressed)
                .with_context(|| format!("Failed to open downloaded file: {}", temp_compressed.display()))?;
    ...
    fn stream_download_and_decompress(
        sess: &Session,
    ...
    fn download_and_parse_worker(
        ...
        let snapshots = temp_loader.parse_snapshot_data(&data)
            ...
            for (line_num, line_result) in reader.lines().enumerate() {
                ...
                if let Ok(row) = serde_json::from_str::<SnapshotRow>(&line) {
                    ...
                    if parts.len() >= 12 {
                        ...
                        if line_num < 3 {
                            eprintln!("Skipping line {} (doesn't match expected format): {}", 
                                line_num + 1, 
                                if line.len() > 200 { format!("{}...", &line[..200]) } else { line });
                        }
                    }
                }
            }
    ...
    fn load_snapshots(&self) -> Result<Box<dyn Iterator<Item = Result<OrderBookSnapshot>> + Send>> {
        ...
        thread::spawn(move || {
            let result = (|| -> Result<()> {
                ...
                println!("Streaming {} files (sequential for chronological order)...", 
