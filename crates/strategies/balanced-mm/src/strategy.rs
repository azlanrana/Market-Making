use mm_core::strategy::{Strategy, StrategyError, OrderIntent, Fill};
...
impl BalancedMMStrategy {
        ...
        if self.volatility_enabled && self.price_history.len() >= 10 {
            ...
            if returns.len() >= 5 {
                ...
                if vol_bps <= self.vol_threshold_bps {
                    ...
                    self.volatility_multiplier = (1.0 + (ratio - 1.0) * 0.5)
                        .clamp(1.0, self.max_spread_multiplier);
                }
            }
        }
    }
    ...
    fn get_effective_spreads(&self) -> Vec<Decimal> {
        let mult = Decimal::from_f64_retain(self.volatility_multiplier)
            .unwrap_or(dec!(1));
        self.base_layer_spreads_bps.iter()
    ...
    fn get_shadow_adjusted_price(
        ...
        let avg_size: Decimal = levels.iter()
            .take(10)
            .map(|(_, amt)| *amt)
            .sum::<Decimal>() / Decimal::from(levels.len().min(10));
        ...
        let threshold = (avg_size * Decimal::from_f64_retain(self.wall_threshold_mult).unwrap_or(dec!(5)))
            .max(min_wall_size);
        let tick = mid_price * Decimal::from_f64_retain(self.shadow_tick_bps / 10000.0).unwrap_or(dec!(0.0001));
...
impl Strategy for BalancedMMStrategy {
    ...
    fn on_orderbook_update(
        ...
        let mid_price = match self.get_micro_price(orderbook)
        ...
        let skew_decimal = Decimal::from_f64_retain(skew_bps).unwrap_or(Decimal::ZERO) / dec!(10000);
        ...
        for (layer_idx, spread_bps) in effective_spreads.iter().enumerate() {
            ...
            let can_place_bid = last_bid_fill.map_or(true, |t| timestamp - t >= self.filled_order_delay);
            
            let last_ask_fill = self.last_ask_fill_times.get(&layer).copied().flatten();
            let can_place_ask = last_ask_fill.map_or(true, |t| timestamp - t >= self.filled_order_delay);
            
            // 1. Apply asymmetric stretch: bid/ask spread multipliers
            let bid_spread_bps = *spread_bps * Decimal::from_f64_retain(bid_stretch).unwrap_or(dec!(1));
            let ask_spread_bps = *spread_bps * Decimal::from_f64_retain(ask_stretch).unwrap_or(dec!(1));
            
            // 2. Dynamic order size
            let bid_amount = self.order_amount * Decimal::from_f64_retain(bid_size_pct).unwrap_or(dec!(1));
            let ask_amount = self.order_amount * Decimal::from_f64_retain(ask_size_pct).unwrap_or(dec!(1));
            ...
            if (active_bid.is_none() || needs_refresh) && can_place_bid && should_place_bids {
                ...
                bid_price = self.get_shadow_adjusted_price(orderbook, OrderSide::Buy, bid_price, mid_price);
                ...
                self.active_bid_orders.insert(layer, Some(format!("bid_L{}_", layer) + &timestamp.to_string()));
            }
            ...
            if (active_ask.is_none() || needs_refresh) && can_place_ask && should_place_asks {
                ...
                ask_price = self.get_shadow_adjusted_price(orderbook, OrderSide::Sell, ask_price, mid_price);
                ...
                self.active_ask_orders.insert(layer, Some(format!("ask_L{}_", layer) + &timestamp.to_string()));
            }
        }
