...
use mm_core::strategy::{Strategy, StrategyError, OrderIntent, Fill, OrderType};
...
pending_scratches: VecDeque<(OrderSide, Decimal, Decimal, f64, u32, Decimal, String, f64, f64)>,
...
let trips = self.layer_round_trips.entry(layer).or_insert_with(VecDeque::new);
...
let entry = self.layer_expectancy_penalty.entry(layer).or_insert(dec!(1.0));
if expectancy < 0.0 {
...
while self.price_history.front().map_or(false, |(t, _)| *t < cutoff) {
...
while self.impulse_price_buffer.front().map_or(false, |(t, _)| *t < ts - impulse_retain) {
    self.impulse_price_buffer.pop_front();
}
let impulse_cooldown_sec = 5.0;
if ts < self.impulse_cooldown_until {
    intents.push(OrderIntent { side: OrderSide::Buy, price: Decimal::ZERO, amount: Decimal::ZERO, order_type: OrderType::Cancel, layer: 0 });
    intents.push(OrderIntent { side: OrderSide::Sell, price: Decimal::ZERO, amount: Decimal::ZERO, order_type: OrderType::Cancel, layer: 0 });
    intents.push(OrderIntent { side: OrderSide::Buy, price: Decimal::ZERO, amount: Decimal::ZERO, order_type: OrderType::Cancel, layer: 1 });
    intents.push(OrderIntent { side: OrderSide::Sell, price: Decimal::ZERO, amount: Decimal::ZERO, order_type: OrderType::Cancel, layer: 1 });
    self.pending_scratches.clear();
    self.scratch_placed = false;
    return intents;
}
if self.impulse_price_buffer.len() >= 3 {
    let price_3s = self.impulse_price_buffer.iter().filter(|(t, _)| *t <= ts - 2.0).last().map(|(_, p)| *p);
    let price_5s = self.impulse_price_buffer.iter().filter(|(t, _)| *t <= ts - 4.0).last().map(|(_, p)| *p);
    let price_10s = self.impulse_price_buffer.iter().filter(|(t, _)| *t <= ts - 9.0).last().map(|(_, p)| *p);
...
let decay = 0.9;
self.toxicity_score = decay * self.toxicity_score
+ (1.0 - decay) * if drift_f64 > 2.5 { 1.0 } else { 0.0 };
...
let inv_pct = if total_value > Decimal::ZERO {
            (base_value / total_value).to_f64().unwrap_or(0.5).clamp(0.0, 1.0)
        } else {
0.5
        };
...
if regime == Regime::Trending && self.last_trending_first_log_ts < ts - 10.0 {
    eprintln!("[TRENDING_FIRST] ts={:.0} trend={:.1} vol={:.2} inv_pct={:.2}%",
        ts, trend_bps, self.current_vol_bps, inv_pct * 100.0);
...
if (inv_pct - self.last_logged_inv_pct).abs() > 0.05 {
    eprintln!("[INV] ts={:.0} inv={:.1}% regime={} trend={:.1}",
        ts, inv_pct * 100.0, self.last_regime_str, trend_bps);
...
let buy_base = current_micro >= mid_price && !buying_toxic
&& (!buy_is_cooling || drift_ok_to_reopen);
let sell_base = current_micro <= mid_price && !selling_toxic
&& (!sell_is_cooling || drift_ok_to_reopen);
let both_would_allow = buy_base && sell_base;
let mut buy_allowed = buy_base && !(both_would_allow && inv_pct >= 0.5) && !regime_block_buys;
let mut sell_allowed = sell_base && !(both_would_allow && inv_pct < 0.5) && !regime_block_sells;
...
let penalty = if (layer == 1 && trend_flatten_ask) || (layer == 1 && trend_flatten_bid) {
dec!(1.0)
                } else {
...
* penalty
* Decimal::from_f64_retain(self.backtest_spread_multiplier).unwrap_or(dec!(1))
                } else {
// L1: floor 15–20 bps — pure rebate capture, no trend-taking
let l1_base = (20.0 + 0.4 * self.current_vol_bps.min(25.0)).max(18.0);
let l1_base_dec =
Decimal::from_f64_retain(l1_base).unwrap_or(dec!(25.0));
l1_base_dec
* Decimal::from_f64_retain(vol_factor * toxicity_mult * bid_regime_mult).unwrap_or(dec!(1))
* penalty
* Decimal::from_f64_retain(self.backtest_spread_multiplier).unwrap_or(dec!(1))
                };
...
l1_base_dec
* Decimal::from_f64_retain(vol_factor * toxicity_mult * ask_regime_mult).unwrap_or(dec!(1))
* penalty
* Decimal::from_f64_retain(self.backtest_spread_multiplier).unwrap_or(dec!(1))
                };
let disabled = self
...
let inv_shift_bps = if inv_pct > 0.30 { -2.0 } else if inv_pct < 0.20 { 2.0 } else { 0.0 };
let inv_shift_dec = Decimal::from_f64_retain(inv_shift_bps / 10000.0).unwrap_or(Decimal::ZERO);
...
if sell_allowed && inv_pct > (1.0 - self.hard_limit_pct) && can_place_ask && l1_ask_ok {
intents.push(OrderIntent {
side: OrderSide::Sell,
...
if let Some(&(ref _side, ref _scratch_price, ref amount, created_ts, ref source_layer, ref initial_price, ref entry_regime, entry_trend_bps, entry_vol_bps)) =
self.pending_scratches.front()
            {
...
let day_str = chrono::Utc.timestamp_opt(ts as i64, 0).single().map(|dt| dt.format("%Y-%m-%d").to_string()).unwrap_or_else(|| "???".into());
...
let edge_bps = Decimal::from_f64_retain(self.pingpong_spread_bps).unwrap_or(Decimal::ZERO) / dec!(10000);
