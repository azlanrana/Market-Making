...
impl MetricsCollector {
    ...
    pub fn record_fill_forensic(&mut self, fill_price: Decimal, mid_price: Decimal, _side: OrderSide, timestamp: f64) {
    ...
    pub fn record_fill(&mut self, side: OrderSide, layer: u32, amount: Decimal, price: Decimal, fees: Decimal) {
        self.record_fill_with_ts(side, layer, amount, price, fees, 0.0);
    }

    pub fn record_fill_with_ts(&mut self, side: OrderSide, layer: u32, amount: Decimal, price: Decimal, fees: Decimal, timestamp: f64) {
        ...
        *self.volume_by_layer.entry(layer).or_insert(Decimal::ZERO) += amount * price;
        ...
        if timestamp > 0.0 {
            if let Some(dt) = chrono::DateTime::<chrono::Utc>::from_timestamp(timestamp as i64, 0) {
                ...
                *self.volume_by_day_by_layer.entry(day.clone()).or_default()
                    .entry(layer).or_insert(Decimal::ZERO) += fill_value;
                *self.fills_by_day_by_layer.entry(day).or_default()
                    .entry(layer).or_insert(0) += 1;
            }
        }
    }
    ...
    pub fn record_trade_pnl_delta(&mut self, pnl: Decimal, layer: u32, timestamp: f64) {
        *self.pnl_by_layer.entry(layer).or_insert(Decimal::ZERO) += pnl;
        if timestamp > 0.0 {
            if let Some(dt) = chrono::DateTime::<chrono::Utc>::from_timestamp(timestamp as i64, 0) {
                let day = dt.format("%Y-%m-%d").to_string();
                *self.pnl_by_day_by_layer.entry(day).or_default()
                    .entry(layer).or_insert(Decimal::ZERO) += pnl;
            }
        }
    }
    ...
    pub fn get_final_stats(&self, portfolio: &crate::portfolio::BacktestPortfolio) -> BacktestStats {
        ...
        let avg_win = if self.winning_trades > 0 {
            (self.gross_profit / Decimal::from(self.winning_trades)).to_f64().unwrap_or(0.0)
        } else {
            0.0
        };
        
        let avg_loss = if self.losing_trades > 0 {
            (self.gross_loss / Decimal::from(self.losing_trades)).to_f64().unwrap_or(0.0)
        } else {
            0.0
        };
        ...
        let return_pct = if initial.portfolio_value > Decimal::ZERO {
            (final_snapshot.total_pnl / initial.portfolio_value).to_f64().unwrap_or(0.0)
        } else {
            0.0
        };
        ...
        let time_periods_per_year = if self.snapshots.len() > 1 {
            let duration = self.snapshots.last().unwrap().timestamp - self.snapshots.first().unwrap().timestamp;
            if duration > 0.0 {
        ...
        if self.snapshots.len() > 1 {
            for i in 1..self.snapshots.len() {
                let value_change = self.snapshots[i].portfolio_value - self.snapshots[i-1].portfolio_value;
        ...
        let realized_pnl_by_day: HashMap<String, Decimal> = self.pnl_by_day_by_layer
            .iter()
            .map(|(day, layers)| {
