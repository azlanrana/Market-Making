...
async fn validate_against_hummingbot() {
    ...
    match runner.run(loader).await {
        Ok(results) => {
            ...
            println!("\nOur Results:");
            println!("  Total Return: {:.4}% (${})", 
                results.stats.return_pct * 100.0, results.stats.total_return);
            println!("  Realized P&L: ${}", results.stats.realized_pnl);
            println!("  Final Inventory: {:.2}%", results.stats.final_inventory_pct * 100.0);
            println!("  Max Drawdown: {:.4}%", results.stats.max_drawdown * 100.0);
            ...
            if return_match {
                ...
                println!("   Expected: ~0.24%, Got: {:.4}%", results.stats.return_pct * 100.0);
            }
            ...
            if drawdown_match {
                ...
                println!("   Expected: ~0.30%, Got: {:.4}%", results.stats.max_drawdown * 100.0);
            }
            ...
            if results.stats.final_inventory_pct > 0.9 {
                println!("⚠️  Inventory imbalance detected ({}%) - matches Hummingbot issue", 
