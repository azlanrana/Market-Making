...
async fn backtest_sftp_24h() {
    ...
    let username =
        std::env::var("CRYPTO_COM_SFTP_USERNAME").unwrap_or_else(|_| "user080".to_string());

    // Key path - can be set via env var or use default project location
    let key_path = std::env::var("CRYPTO_COM_SFTP_KEY_PATH").unwrap_or_else(|_| {
