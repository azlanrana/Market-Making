...
async fn backtest_s3_7days() {
    ...
    let bucket = std::env::var("S3_BUCKET").expect("S3_BUCKET environment variable must be set");
    ...
    println!(
        "Strategy: Multi-layer MM (10/20/35 bps), {} orders, ping-pong={}",
        order_amount, pair_config.pingpong_enabled
    );
    ...
    match runner.run(loader).await {
        Ok(results) => {
            ...
            let avg_fill_size = if results.simulator_stats.total_fills > 0 {
                results.stats.total_volume
                    / rust_decimal::Decimal::from(results.simulator_stats.total_fills)
            } else {
            ...
            let fill_rate = if results.simulator_stats.filled_orders > 0 {
                (results.simulator_stats.total_fills as f64
                    / results.simulator_stats.filled_orders as f64)
                    * 100.0
            } else {
                0.0
            };
            ...
            println!("\n--- Inventory Extremes ---");
            println!(
                "Max inventory reached: {:.2}%",
                results.stats.max_inventory_reached * 100.0
            );
            println!(
                "Min inventory reached: {:.2}%",
            ...
            if !results.stats.fill_gap_bps_by_hour.is_empty() {
                let mut hours: Vec<u32> =
                    results.stats.fill_gap_bps_by_hour.keys().copied().collect();
                hours.sort();
                for h in hours {
                    let (sum_bps, count) = results
                        .stats
                        .fill_gap_bps_by_hour
                        .get(&h)
                        .unwrap_or(&(0.0, 0));
                    let avg_bps = if *count > 0 {
                        sum_bps / *count as f64
                    } else {
                        0.0
                    };
                    let pnl = results
                        ...
                        .unwrap_or(0.0);
                    println!(
                        "  Hour {}: avg fill gap {:+.1} bps ({} fills), P&L ${:+.0}",
                        h, avg_bps, count, pnl
                    );
                }
            } else {
            ...
            println!("\n--- Temporal Analysis ---");
            println!(
                "Max drawdown duration: {:.2} hours",
                results.stats.max_drawdown_duration / 3600.0
            );

            // Show top 5 best/worst hours
            let mut hour_pnl: Vec<(u32, f64)> = results
                .stats
                .pnl_by_hour
                .iter()
            ...
            let mut day_pnl: Vec<(String, f64)> = results
                .stats
                .pnl_by_day
                .iter()
            ...
            if results.stats.final_inventory_pct > 0.4 && results.stats.final_inventory_pct < 0.6 {
                println!(
                    "✅ Inventory well balanced: {:.2}%",
                    results.stats.final_inventory_pct * 100.0
                );
            } else {
                println!(
                    "⚠️  Inventory imbalance: {:.2}%",
                    results.stats.final_inventory_pct * 100.0
                );
            }
        ...
        Err(e) => {
            ...
            eprintln!(
                "5. Set MAX_FILES to cap object count after filters (optional; omit for no cap)"
            );
