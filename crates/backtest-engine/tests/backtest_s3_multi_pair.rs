...
async fn backtest_s3_btcusdt() {
    run_backtest_for_pair(
        "BTC_USDT",
        "backtest-btcusdt-2025",
        "2023/10/25/cdc/BTC_USDT/",
    )
    .await;
}
...
async fn backtest_s3_ethusdt() {
    run_backtest_for_pair(
        "ETH_USDT",
        "backtest-ethusdt-2025",
        "2023/10/25/cdc/ETH_USDT/",
    )
    .await;
}
...
async fn backtest_s3_single_bucket() {
    ...
    run_backtest_for_pair(
        "BTC_USDT",
        "backtest-data",
        "BTC_USDT/2023/10/25/cdc/BTC_USDT/",
    )
    .await;
}
...
async fn run_backtest_for_pair(pair: &str, bucket: &str, prefix: &str) {
    ...
    let strategy = BalancedMMStrategy::new_with_config(dec!(0.1), 5.0, 0.5)
