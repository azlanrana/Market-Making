...
Rebate line stays **~+0.75 bps** on volume; **spread capture** turns negative in weaker months (late year in this run set). See `LogsETHUSD.md` for narrative and follow-on experiments.
...
**Read:** Rebate **~+0.75 bps** is stable; **spread capture** and **net edge** swing with **tape density** (e.g. thin March vs rich January — see `LogsBTCUSD-RebateMM.md` S3 inventory notes). Sharpe collapses in stressed months when spread leakage dominates.

---

## How to interpret

- **Simulation only** — queue model, fees, and data completeness affect all numbers; **not** live trading results.
- **Sum of months ≠ path-dependent year sim** — no single portfolio trajectory across the year; totals are for **aggregated economic scale**, not one continuous Sharpe.
- **Vendor tape** — month-to-month file counts and bytes differ (e.g. BTC Jan vs Aug); compare months with that in mind.
